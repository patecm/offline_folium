...
- Right map: H3 cells colored by score (0-1, green to red)
...
def create_colormap_score(value: float) -> str:
    """Create seismic-like colormap for score (blue -> white -> red, centered at 0.5)."""
    # Clamp to 0-1
...
def create_score_map(df_hour: pd.DataFrame, center_lat: float, center_lng: float) -> str:
    ...
    from branca.colormap import LinearColormap
    THRESHOLD = 0.3
    # Create seismic-like colormap
    colors = ['#0000ff', '#ffffff', '#ff0000']
    colormap = LinearColormap(
        colors=colors,
        index=[0, THRESHOLD, 1.0],
        vmin=0.0,
        vmax=1.0,
        caption='Score'
    )
